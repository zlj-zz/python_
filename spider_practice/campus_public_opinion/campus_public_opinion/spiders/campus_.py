...
class CampusSpider(scrapy.Spider):
    ...
    crawl_page = 2  # number of pages crawled
    ...
    def parse_detail(self, response):
        ...
        title = response.xpath('//div[@class="core_title core_title_theme_bright"]/h1/@title').extract()
        item["title"] = title[0] if title else ""
        reply_number = response.xpath('//ul[@class="l_posts_num"]/li[@class="l_reply_num"]/span/text()').extract()
        item["reply_number"] = reply_number[0] if reply_number else 0
        contents = response.xpath('//div[contains(@class,"l_post j_l_post")]').extract()
        content = ""
        for comment in contents:
            comment = etree.HTML(comment)
            comment_user = comment.xpath('//div[@class="d_author"]/ul/li[@class="d_name"]/a/text()')[0]
